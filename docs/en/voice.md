...
1. Open **Settings → Model routing** and choose Browser / system voice for the TTS role.
...
1. Open **Settings → AI Providers**, add the service, set Base URL to `http://127.0.0.1:8880/v1`, leave API key empty, and add the model `kokoro`.
2. Open **Settings → Model routing** and assign this provider and `kokoro` to TTS.
...
4. Open **Settings → Advanced → Text-to-speech**, choose the default, GM, and player voices, and select Save and test.
...
Start the GPT-SoVITS HTTP API using its own instructions, then select GPT-SoVITS under **Settings → Model routing** and configure its provider. Each personal voice also needs a reference WAV, an exact transcript, and the prompt language:
...
1. In **Settings → Model routing**, open the Speech recognition model card, choose OpenAI-compatible, and select a saved provider and model.
2. In **Settings → Advanced → Speech recognition**, set the timeout and save.
